...
import { buildProjectionLineFromMatrix } from './utils/buildProjectionLineFromMatrix';
import { intersectProjectionLineWithSegment } from './utils/intersectProjectionLineWithSegment';
import { isCloserToCamera } from './utils/isCloserToCamera';
import { removeDullEdges } from './utils/removeDullEdges';
import { removeFlatTriangles } from './utils/removeFlatTriangles';
...
export async function algo({
  ...
  for (const triangle of triangles) {
    ...
    for (const otherTriangle of processedTriangles) {
      ...
      for (
        let currentTriangleEdgeIndex = 0;
        currentTriangleEdgeIndex < 3;
        currentTriangleEdgeIndex++
      ) {
        const newSegments2dForCurrentTriangle: Segment2d[] = [];
        const newSegments3dForCurrentTriangle: Segment3d[] = [];

        const currentTriangleEdge =
          currentTriangle.edges[currentTriangleEdgeIndex];
        for (
          let currentTriangleEdgeSegmentIndex = 0;
          currentTriangleEdgeSegmentIndex <
          currentTriangleEdge.edgeSegments2d.length;
          currentTriangleEdgeSegmentIndex++
        ) {
          const currentTriangleEdgeSegment =
            currentTriangleEdge.edgeSegments2d[currentTriangleEdgeSegmentIndex];
          const currentTriangleEdgeSegment3d =
            currentTriangleEdge.edgeSegments3d[currentTriangleEdgeSegmentIndex];

          if (
            currentTriangleEdgeSegment[0].distanceToSquared(
              currentTriangleEdgeSegment[1],
            ) <= MIN_SEGMENT_WIDTH_SQ
          ) {
            continue;
          }
          ...
          for (
            let otherTriangleEdgeIndex = 0;
            otherTriangleEdgeIndex < 3;
            otherTriangleEdgeIndex++
          ) {
            const otherTriangleEdge =
              otherTriangle.edges[otherTriangleEdgeIndex];
            const newSegments2dForOtherTriangle: Segment2d[] = [];
            const newSegments3dForOtherTriangle: Segment3d[] = [];
            for (
              let otherTriangleEdgeSegmentIndex = 0;
              otherTriangleEdgeSegmentIndex <
              otherTriangleEdge.edgeSegments2d.length;
              otherTriangleEdgeSegmentIndex++
            ) {
              const otherTriangleEdgeSegment =
                otherTriangleEdge.edgeSegments2d[otherTriangleEdgeSegmentIndex];
              const otherTriangleEdgeSegment3d =
                otherTriangleEdge.edgeSegments3d[otherTriangleEdgeSegmentIndex];

              if (
                otherTriangleEdgeSegment[0].distanceToSquared(
              ...
              if (
                currentTriangleEdgeSegment[0].equals(
                  otherTriangleEdgeSegment[0],
                ) ||
                currentTriangleEdgeSegment[0].equals(
                  otherTriangleEdgeSegment[1],
                ) ||
                currentTriangleEdgeSegment[1].equals(
                  otherTriangleEdgeSegment[0],
                ) ||
                currentTriangleEdgeSegment[1].equals(
                  otherTriangleEdgeSegment[1],
              ...
              if (intersectionPoint) {
                ...
                const currentEdgePoint = intersectProjectionLineWithSegment(
                  projectionLine.origin,
                  projectionLine.direction,
                  currentTriangleEdgeSegment3d,
                );
                const otherEdgePoint = intersectProjectionLineWithSegment(
                  projectionLine.origin,
                  projectionLine.direction,
                  otherTriangleEdgeSegment3d,
                );

                const currentIsCloser = isCloserToCamera(
                  currentEdgePoint,
                  otherEdgePoint,
                  viewProjectionMatrix,
                );
                ...
                if (currentIsCloser) {
                  const otherTrianglePointASide = pointSideOfSegment(
                    currentTriangleEdgeSegment[0],
                    currentTriangleEdgeSegment[1],
                    otherTriangleEdgeSegment[0],
                  );
                  const otherTrianglePointBSide = pointSideOfSegment(
                    currentTriangleEdgeSegment[0],
                    currentTriangleEdgeSegment[1],
                    otherTriangleEdgeSegment[1],
                  );
                  ...
                  const currentTrianglePoint0Side = pointSideOfSegment(
                    otherTriangleEdgeSegment[0],
                    otherTriangleEdgeSegment[1],
                    currentTriangleEdgeSegment[0],
                  );
                  const currentTrianglePoint1Side = pointSideOfSegment(
                    otherTriangleEdgeSegment[0],
                    otherTriangleEdgeSegment[1],
                    currentTriangleEdgeSegment[1],
                  );
            ...
            console.assert(
              newSegments2dForOtherTriangle.length ===
                newSegments3dForOtherTriangle.length,
            );

            if (newSegments2dForOtherTriangle.length) {
              if (!touched[otherTriangleEdgeIndex]) {
                OTHER_TRIANGLE_NEW_DATA.edges[
                  otherTriangleEdgeIndex
                ].edgeSegments2d = newSegments2dForOtherTriangle;
                OTHER_TRIANGLE_NEW_DATA.edges[
                  otherTriangleEdgeIndex
                ].edgeSegments3d = newSegments3dForOtherTriangle;
                touched[otherTriangleEdgeIndex] = true;
              } else {
                OTHER_TRIANGLE_NEW_DATA.edges[
                  otherTriangleEdgeIndex
                ].edgeSegments2d.push(...newSegments2dForOtherTriangle);
                OTHER_TRIANGLE_NEW_DATA.edges[
                  otherTriangleEdgeIndex
                ].edgeSegments3d.push(...newSegments3dForOtherTriangle);
              }
            }
          }
        }
        if (newSegments2dForCurrentTriangle.length) {
          CURRENT_TRIANGLE_NEW_DATA.edges[
            currentTriangleEdgeIndex
          ].edgeSegments2d = newSegments2dForCurrentTriangle;
          CURRENT_TRIANGLE_NEW_DATA.edges[
            currentTriangleEdgeIndex
          ].edgeSegments3d = newSegments3dForCurrentTriangle;
        }
      }
    ...
    const nextProgress =
      (procesed / ((triangles.length * (triangles.length - 1)) / 2)) * 100;
