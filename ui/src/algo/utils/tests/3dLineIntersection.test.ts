...
describe('line', () => {
  it('case 1', () => {
    ...
    const line1Projected = line1.map(point =>
      worldToProjection(point, viewProjectionMatrix),
    );
    const line2Projected = line2.map(point =>
      worldToProjection(point, viewProjectionMatrix),
    );

    expect(line1Projected).toEqual([
      new THREE.Vector2(-1, -1),
      new THREE.Vector2(1, 1),
    ]);
    expect(line2Projected).toEqual([
      new THREE.Vector2(1, -1),
      new THREE.Vector2(-1, 1),
    ]);
    ...
    const line1Point = intersectProjectionLineWithSegment(
    ...
    const line2Point = intersectProjectionLineWithSegment(
      projectionLine.origin,
      projectionLine.direction,
      line2,
    );
