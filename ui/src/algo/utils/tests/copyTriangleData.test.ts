...
describe('copyTriangleData', () => {
  ...
  it('copied obj has different references', () => {
    ...
    for (let i = 0; i < triangle.edges.length; i++) {
      ...
      expect(triangle.edges[i].edgeSegments2d).not.toBe(
        copy.edges[i].edgeSegments2d,
      );
      for (let j = 0; j < triangle.edges[i].edgeSegments2d.length; j++) {
        expect(triangle.edges[i].edgeSegments2d[j]).not.toBe(
          copy.edges[i].edgeSegments2d[j],
        );
        for (let k = 0; k < triangle.edges[i].edgeSegments2d[j].length; k++) {
          expect(triangle.edges[i].edgeSegments2d[j][k]).not.toBe(
            copy.edges[i].edgeSegments2d[j][k],
          );
          expect(triangle.edges[i].edgeSegments3d[j][k]).not.toBe(
            copy.edges[i].edgeSegments3d[j][k],
          );
        }
      }
    }
