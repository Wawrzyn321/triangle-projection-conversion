...
import type { Segment2d, TriangleData } from '@/pages/Main/types';

// todo used?
export function removeInsideTriangles(
  processedTriangles: TriangleData[],
  worldToProjectionBound: (point: THREE.Vector3) => THREE.Vector2,
  camera: THREE.Camera,
          ...
          edgeSegments2d: edgeSegments2d.filter(segment => {
            for (let i = 0; i < processedTriangles.length; i++) {
              ...
              if (isInTriangle(segment[0]) && isInTriangle(segment[1])) {
                const segmentMid = rest.start
                  .clone()
                  .add(rest.end)
                  .divideScalar(2);
                const triangleMid = triangle.edges[0].start
                  .clone()
                  .add(triangle.edges[1].start)
                  .add(triangle.edges[2].start)
                  .divideScalar(3);
                const segmentMidCameraDistance = segmentMid
                  .clone()
                  .project(camera).z;
                const triangleMidCameraDistance = triangleMid
                  .clone()
                  .project(camera).z;
