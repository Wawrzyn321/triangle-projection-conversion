...
export function removeDullEdges(processedTriangles: TriangleData[], minAngleDegrees = MIN_ANGLE_DEGREES): TriangleData[] {
...
function handle(ORIGINAL_TRIANGLE: TriangleData, OTHER_TRIANGLE: TriangleData, minAngleRads: number) {
  for (let ORIGINAL_TRIANGLE_edgeIndex = 0; ORIGINAL_TRIANGLE_edgeIndex < 3; ORIGINAL_TRIANGLE_edgeIndex++) {
    for (let OTHER_TRIANGLE_edgeIndex = 0; OTHER_TRIANGLE_edgeIndex < 3; OTHER_TRIANGLE_edgeIndex++) {
      if (
        ...
        const original = ORIGINAL_TRIANGLE.edges[(ORIGINAL_TRIANGLE_edgeIndex + 1) % 3].end;
        const other = OTHER_TRIANGLE.edges[(OTHER_TRIANGLE_edgeIndex + 1) % 3].end;

        if (original.equals(common1) || original.equals(common2) || other.equals(common1) || other.equals(common2) || original.equals(other)) {
          throw Error('brzydko')
        }
